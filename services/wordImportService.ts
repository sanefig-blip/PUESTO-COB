...
function parsePersonnelExcel(sheet: XLSX.WorkSheet): Partial<Schedule> | null {
...
function parseTemplateExcel(sheet: XLSX.WorkSheet): Partial<Schedule> {
...
export const parseFullUnitReportFromExcel = (fileBuffer: ArrayBuffer): UnitReportData | null => {
    ...
    const blockStartsByCol = new Map<number, number[]>();
    const blockStartKeywords = [
        'ESTACION', 'ESTACIÓN', 'DTO.', 'DESTAC.', 'DESTACAMENTO', 'BRIGADA', 
        'OFICINA', 'COMPAÑIA', 'COMPANIA', 'DIVISIÓN', 'DIVISION', 'TRANSPORTE', 'URIP', 'O.C.O.B.'
    ];
...
const parseFullSchedule = (lines: string[]): Partial<Schedule> | null => {
    ...
    let currentService: Service | null = null;
    let currentAssignment: Partial<Assignment> & { details?: string[] } = {};
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (staffMatch) {
            ...
            const rank = rankMatch ? rankMatch[0].toUpperCase() as Rank : 'OTRO';
