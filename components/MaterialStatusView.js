...
const MaterialStatusView = ({ materialsData }) => {
  ...
  const [selectedConditions, setSelectedConditions] = useState(new Set(allConditions));
  ...
  return (
    React.createElement("div", { className: "animate-fade-in grid grid-cols-1 lg:grid-cols-4 gap-6" },
      ...
      React.createElement("div", { className: "lg:col-span-3 bg-zinc-800/60 p-4 rounded-xl" },
        React.createElement("h2", { className: "text-2xl font-bold text-white mb-4" }, `Materiales Filtrados (${filteredMaterials.length})`),
          ...
          React.createElement("table", { className: "w-full text-left" },
            React.createElement("thead", { className: "sticky top-0 bg-zinc-800" },
              React.createElement("tr", { className: "text-sm text-zinc-400" },
                React.createElement("th", { className: "p-2" }, "Material"),
                React.createElement("th", { className: "p-2 text-center" }, "Cantidad"),
                React.createElement("th", { className: "p-2" }, "Condición"),
                React.createElement("th", { className: "p-2" }, "Ubicación"),
                React.createElement("th", { className: "p-2" }, "Ubicación Interna")
              )
            ),
            React.createElement("tbody", null,
              filteredMaterials.length > 0 ? filteredMaterials.map(material => (
                React.createElement("tr", { key: material.id + material.locationName, className: "border-t border-zinc-700 hover:bg-zinc-700/50" },
                  React.createElement("td", { className: "p-2 font-semibold text-zinc-200" }, material.name),
                  React.createElement("td", { className: "p-2 text-center font-mono text-white" }, material.quantity),
                  React.createElement("td", { className: "p-2" },
                    React.createElement("span", { className: `px-2 py-1 text-xs font-semibold rounded-full ${getConditionColor(material.condition)}` },
                      material.condition
                    )
                  ),
                  React.createElement("td", { className: "p-2 text-zinc-400 text-sm" }, material.locationName),
                  React.createElement("td", { className: "p-2 text-zinc-300" }, material.location || '-')
                )
              )) : (
                React.createElement("tr", null,
                    React.createElement("td", { colSpan: 5, className: "text-center py-12 text-zinc-500" },
